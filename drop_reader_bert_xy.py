...
class DROPReaderNew(DatasetReader):
    def __init__(
        ...
        only_program_supervised: bool = False,
        ...
        self._tag_indexers = {"tags":  SingleIdTokenIndexer(namespace='tags')}
        self._sem_indexers = {"sems":  SingleIdTokenIndexer(namespace='sems')}
        self._relaxed_span_match = relaxed_span_match
        self._do_augmentation = do_augmentation
        self.question_length_limit = question_length_limit
        self.only_program_supervised = only_program_supervised
    ...
    def _read(self, file_path: str):
        ...
        for passage_id, passage_info in dataset.items():
            ...
            if constants.syntax_label:
                p_syntax_label = passage_info[constants.syntax_annotation]
            else:
                p_syntax_label = None
            if constants.syntax_parent_label:
                p_parent_syntax_label = passage_info[constants.parent_annotation]
            else:
                p_parent_syntax_label = None
            if constants.semantic_label:
                p_semantic_label = passage_info[constants.semantic_annotation]
            else:
                p_semantic_label = None
            ...
            for qa in passage_info[constants.qa_pairs]:
                ...
                if constants.syntax_label:
                    q_syntax_label = qa[constants.syntax_annotation]
                else:
                    q_syntax_label = None

                if constants.syntax_parent_label:
                    q_parent_syntax_label = qa[constants.parent_annotation]
                else:
                    q_parent_syntax_label = None

                if constants.semantic_label:
                    q_semantic_label = qa[constants.semantic_annotation]
                else:
                    q_semantic_label = None
                ...
                instance = self.text_to_instance(
                    ...
                    p_syntax_label,
                    q_syntax_label,
                    p_parent_syntax_label,
                    q_parent_syntax_label,
                    p_semantic_label,
                    q_semantic_label
                )

                if self.only_program_supervised:
    ...
    def text_to_instance(
        ...
        p_syntax_label: List[str] = None,
        q_syntax_label: List[str] = None,
        p_parent_syntax_label: List[str] = None,
        q_parent_syntax_label: List[str] = None,
        p_semantic_label: List[str] = None,
        q_semantic_label: List[str] = None,
    ) -> Union[Instance, None]:
        ...
        if constants.syntax_label:
            passage_syntax_label = [p_syntax_label[i] for i, _ in enumerate(p_tokenidx2wpidx) for _ in
                                    range(len(p_tokenidx2wpidx[i]))]
            question_syntax_label = [q_syntax_label[i] for i, _ in enumerate(q_tokenidx2wpidx) for _ in
                                     range(len(q_tokenidx2wpidx[i]))]

            passage_syntax_label = passage_syntax_label[0:len(passage_wps_tokens)-1]
            passage_syntax_label.append("[SEP]")
            assert len(passage_syntax_label) == len(passage_wps_tokens)
            question_syntax_label = question_syntax_label[0:max_question_len]
            q_syntax_label_len = len(question_syntax_label)
            q_syntax_label_pad_len = max_question_len - q_syntax_label_len
            question_syntax_label.extend(["[PAD]"] * q_syntax_label_pad_len)
            assert len(question_syntax_label) == len(question_wps_tokens)

            passage_syntax = [Token(text=t) for t in passage_syntax_label]
            question_syntax = [Token(text=t) for t in question_syntax_label]
            fields["passage_syntax"] = TextField(passage_syntax, self._tag_indexers)
            fields["question_syntax"] = TextField(question_syntax, self._tag_indexers)
        if constants.syntax_parent_label:
            passage_syntax_label = [p_parent_syntax_label[i] for i, _ in enumerate(p_tokenidx2wpidx) for _ in
                                    range(len(p_tokenidx2wpidx[i]))]
            question_syntax_label = [q_parent_syntax_label[i] for i, _ in enumerate(q_tokenidx2wpidx) for _ in
                                     range(len(q_tokenidx2wpidx[i]))]
            ...
            passage_syntax = [Token(text=t) for t in passage_syntax_label]
            question_syntax = [Token(text=t) for t in question_syntax_label]
            fields["passage_parent_syntax"] = TextField(passage_syntax, self._tag_indexers)
            fields["question_parent_syntax"] = TextField(question_syntax, self._tag_indexers)
        if constants.semantic_label:
            passage_semantic_label = [p_semantic_label[i] for i, _ in enumerate(p_tokenidx2wpidx) for _ in
                                      range(len(p_tokenidx2wpidx[i]))]
            question_semantic_label = [q_semantic_label[i] for i, _ in enumerate(q_tokenidx2wpidx) for _ in
                                       range(len(q_tokenidx2wpidx[i]))]

            passage_semantic_label = passage_semantic_label[0:len(passage_wps_tokens)-1]
            passage_semantic_label.append("[SEP]")
            assert len(passage_semantic_label) == len(passage_wps_tokens)
            question_semantic_label = question_semantic_label[0:max_question_len]
            q_semantic_label_len = len(question_semantic_label)
            q_semantic_label_pad_len = max_question_len - q_semantic_label_len
            question_semantic_label.extend(["[PAD]"] * q_semantic_label_pad_len)
            assert len(question_semantic_label) == len(question_wps_tokens)

            passage_semantic = [Token(text=t) for t in passage_semantic_label]
            question_semantic = [Token(text=t) for t in question_semantic_label]
            fields["passage_semantic"] = TextField(passage_semantic, self._sem_indexers)
            fields["question_semantic"] = TextField(question_semantic, self._sem_indexers)
